...
/*
  ...
  const Sales_data& ex19_19(const std::vector<Sales_data>&vctr,const int& threshold){
       double(Sales_data::*it)()const;
	  
       it=&Sales_data::ave_price;
          auto iter=find_if(vctr.cbegin(),vctr.cend(),[it,
		 &threshold](const Sales_data& item)
          ->bool{return std::mem_fn(it)(item)>threshold;});
          return *iter;
   }
int main(){

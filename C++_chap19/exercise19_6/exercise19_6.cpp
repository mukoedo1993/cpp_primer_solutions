...
NotQuery::eval(const TextQuery& text)const
{
   ...
   auto sz=result.get_file()->size();
    for(size_t n=0;n!=sz;++n){
     ...
     if(beg==end||*beg!=n)//We must put *beg!=n on the right side, otherwise, 
     //end might be dereferenced and an error will occur!!!!!!!
...
that number to ret_lines. If the number is in result, we increment beg, which is our
iterator into result.
*/

int main(){
    std::ifstream input1("ex19_6i.txt");
    if(!input1.is_open()){std::cerr<<"Error! File not found!\n";exit(-1);}
   
    std::ofstream output1("ex19_6o1.txt",std::ofstream::out| std::ofstream::app);
...
AndQuery andqry(Ob1,Ob2);
OrQuery orqry(Ob1,Ob2);
    Query_base *Qb_ptr=&andqry;//At here, we need to see that Qb_ptr's dynamic type is AndQuery.
    Query_base &Qb_lvalue_rf=andqry;
    Query_base &&Qb_rvalue_rf=std::move(andqry);
      Query_base *Qb_ptr1=&orqry;//At here, we need to see that Qb_ptr's dynamic type is AndQuery.
    Query_base &Qb_lvalue_rf1=orqry;
    Query_base &&Qb_rvalue_rf1=std::move(orqry);

    if(AndQuery *And_ptr=dynamic_cast<AndQuery*>(Qb_ptr))
    std::cout<<"dynamic_cast works at this AndQuery pointer case at this time!\n";
    if(OrQuery *Or_ptr=dynamic_cast<OrQuery*>(Qb_ptr1))
    std::cout<<"dynamic_cast works at this OrQuery* pointer case at this time!\n";
    try
    {AndQuery And_ptr=dynamic_cast<AndQuery&>(Qb_lvalue_rf);
    std::cout<<"dynamic cast works at this lvalue reference case at this time!\n";}
    catch(std::bad_cast bd)
    {std::cerr<<bd.what()<<std::endl;}
    try
    {OrQuery Or_ptr=dynamic_cast<OrQuery&>(Qb_lvalue_rf1);
    std::cout<<"dynamic cast works at this lvalue reference case at this time!\n";}
    catch(std::bad_cast bd)
    {std::cerr<<bd.what()<<std::endl;}
   
        try
    {AndQuery And_ptr=dynamic_cast<AndQuery&&>(Qb_rvalue_rf);
    std::cout<<"dynamic cast works at this rvalue reference case at this time!\n";}
    catch(std::bad_cast bd)
    {std::cerr<<bd.what()<<std::endl;}
       try
    {OrQuery  Or_ptr=dynamic_cast<OrQuery&&>(Qb_rvalue_rf1);
    std::cout<<"dynamic cast works at this rvalue reference case at this time!\n";}
    catch(std::bad_cast bd)
    {std::cerr<<bd.what()<<std::endl;}
}
...
dynamic_cast works at this AndQuery pointer case at this time!
dynamic_cast works at this OrQuery* pointer case at this time!
dynamic cast works at this lvalue reference case at this time!
dynamic cast works at this lvalue reference case at this time!
dynamic cast works at this rvalue reference case at this time!
dynamic cast works at this rvalue reference case at this time!
*/
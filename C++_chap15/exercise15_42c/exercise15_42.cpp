...
class QueryResult {
    ...
    friend std::ostream& print1(std::ostream&, const QueryResult&,std::set<line_no> ln);
public:
...
std::ostream& print1(std::ostream& os, const QueryResult& qr,std::set<line_no> ln)
{
    std::set<line_no>mkft1; std::set<line_no>mkft2(qr.lines->begin(),qr.lines->end());
    set_intersection(mkft2.begin(),mkft2.end(),ln.begin(),ln.end(),
    std::inserter(mkft1,mkft1.end()));
	os << qr.sought << " occurs " << mkft1.size() << " "
		<< make_plural(mkft1.size(), "time", "s") << std::endl;
	for (auto num : mkft1)
...
//exercise 15_42: 
int main(){
    std::set<line_no>v_seek;
    line_no i;
    while(std::cin>>i)v_seek.insert(i+1);
    std::cout<<std::endl;
    std::ifstream input1("ex15_39i.txt");
    if(!input1.is_open()){std::cerr<<"Error! File not found!\n";exit(-1);}
   
    std::ofstream output1("ex15_42o1.txt",std::ofstream::out);//| std::ofstream::app);
    output1<<"The range of numbers (of lines) we are going to seek: \n";
    for_each(v_seek.begin(),v_seek.end(),[&](line_no l1){output1<<l1-1<<" ";});
    output1<<std::endl;
     output1<<std::endl<<"Our first requirement: "<<std::endl;
    output1<<"Time: "<<__TIME__<<"\nDate: "<<__DATE__<<"\n\n";
    TextQuery Ob1_(input1);
   
      Query Ob1(""),Ob2("to"),Ob3("is");
    Query q(Ob1|Ob2);
    Query q1(Ob2|Ob1);
    Query q2(Ob1|Ob1);
    Query q_contrl(Ob2);
    ...
    QueryResult rslt1=q.eval(Ob1_);
     QueryResult rslt2=q1.eval(Ob1_);
      QueryResult rslt3=q2.eval(Ob1_);
      QueryResult contrl=q_contrl.eval(Ob1_);   
    output1<<"Time: "<<__TIME__<<"\nDate: "<<__DATE__<<"\n\n";
      output1<<std::endl<<"Our first requirement: "<<std::endl;
    print1(output1,rslt1,v_seek);
      output1<<std::endl<<"Our Second requirement: "<<std::endl;
      print1(output1,rslt2,v_seek);
        output1<<std::endl<<"Our third requirement: "<<std::endl;
        print1(output1,rslt3,v_seek);
         output1<<std::endl<<"Our control group: "<<std::endl;
        print1(output1,contrl,v_seek);    
}

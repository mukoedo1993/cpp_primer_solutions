...
auto lambda1 = [](connection* p)->void {disconnect(*p); };
...
void f_yaju(destination& d)
{
	...
	shared_ptr<connection>p(&c,lambda1);//end_connection is called to free p.
}
...
int main()
{
	...
	f_yaju(*Hengyang);//f_yaju only destroyed object c inside of class. It doesn't touch the connec object in the main block.

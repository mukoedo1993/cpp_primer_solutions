...
*iterators and in smart pointer classes. We can logically add these operators to our
*StrBlobPtr Class as well:
*
*/
...
*returns a reference to the element denoted by curr. The arrow operator avoids
* doing any work of its own by calling the dereference operator and returning the address
*of the element by that operator.
*/
/*It is worth noting that we've defined these operators as const members.
*Unlike the incrmeent and decrement operators, fetching an element doesn't change the state
*of a StrBlobPtr. Also note that these operators return a reference or pointer to nonconst string.
* 
*/
...
//we could define operator* to do whatever processing we like.
//When we write point->mem, point must either be a pointer to a class object or it 
//must be an object of a class with an overloaded operator->. Depending on the type
//of point, writing point->mem is equivalent to
...
vector<int>* operator->()
{
    std::cout<<"Call an overloaded operator->()!\n";
    return &k1;
}
vector<int>k1={12,3,56,74};
};
...
int main()
{
    ...
    *p="okay";//assigns the first element in a1
    std::cout<<p->size()<<std::endl;//prints 4, the size of first element in a1
    std::cout<<(*p).size()<<std::endl;//equivalent to p->size()
    test1 tst1,*tst2=&tst1;
    std::cout<<*tst1<<std::endl;
    std::cout<<(tst1->size())<<std::endl;
    std::cout<<(tst2->k1[2])<<std::endl;
